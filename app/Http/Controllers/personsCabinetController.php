...
class personsCabinetController extends Controller
{
    public function index(Request $request)
    {
        ...
        $persTests = DB::table('pers_tests')
            ->where('pers_tests.pers_id', session('person_id'))
        ...
        $persEvent = DB::table('pers_events')
            ...
            ->where('pers_events.pers_id', session('person_id'))
            ->select('pers_events.*', 'events.name', 'events.date_start', 'events.date_end')
            ->orderby('pers_events.id', 'desc')
            ->get();
            $person = DB::table('persons')->where('id', '=' , session('person_id'))->first();
        ...
        $pers = DB::table('persons')->where('id', session('person_id'))->where('is_block', 'F')->first();
    ...
    public function savevent(Request $request)
    {
        ...
        $pe = DB::table('pers_events')->where('pers_id', session('person_id'))->where('event_id', $eid)->count();

        if ($pe == 0) {

            DB::table('persons')->where('id', session('person_id'))->update(['study_place' => $place_study]);
    ...
    function createPdf(Request $request)
    {
        ...
        $pers = DB::table('persons')->where('id', $pers_event->pers_id)->first();
    ...
    {
        $pers = DB::table('persons')->where('id', session('person_id'))->first();
