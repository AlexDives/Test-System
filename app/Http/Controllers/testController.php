...
class testController extends Controller
{
    public function start(Request $request)
    {
        ...
        if ($test != null) {
            ...
            testController::generateTest($request->ptid);
        ...
        else echo '<script>location.replace("/pers/cabinet");</script>'; exit;
    }

    public function generateTest($ptid)
    {
        if (!DB::table('pers_test_details')->where('pers_test_id', $ptid)->exists()) {
            $questions = DB::table('questions')
                ...
                ->where('pers_tests.id', $ptid)->where('question_details.type', 'que')
                ->select(
                    'questions.id',
                    'questions.ball',
    ...
    public function result(Request $request)
    {
        $test = DB::table('tests')
            ...
            ->where('pers_tests.id', session('ptid'))
            ->first();
